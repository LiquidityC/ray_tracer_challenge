...
#define TEST_ASSERT_EQUAL_COLOR(e, c) \
    TEST_ASSERT_FLOAT_WITHIN(epsilon, e.r, c.r); \
    TEST_ASSERT_FLOAT_WITHIN(epsilon, e.g, c.g); \
    TEST_ASSERT_FLOAT_WITHIN(epsilon, e.b, c.b)

#define TEST_ASSERT_EQUAL_MATERIAL(e, m) \
    TEST_ASSERT_EQUAL_VEC4(e.color, m.color); \
    TEST_ASSERT_FLOAT_WITHIN(epsilon, e.ambient, m.ambient); \
    TEST_ASSERT_FLOAT_WITHIN(epsilon, e.diffuse, m.diffuse); \
    TEST_ASSERT_FLOAT_WITHIN(epsilon, e.specular, m.specular); \
    TEST_ASSERT_FLOAT_WITHIN(epsilon, e.shininess, m.shininess)
